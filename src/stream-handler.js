  ...
  await fetchWithKeyRotation(req, res, reqId, startTime, body, {
    ...
    onSuccess: (bodyStream, forwardHeaders, keyIdx) => {
      ...
      if (clientGone || res.destroyed || res.writableEnded) {
        console.warn(
          `[REQ ${reqId}] client gone — discarding upstream response`,
        );
        bodyStream.destroy();
        return;
      }
    ...
    onAllExhausted: () => {
      /* writeRateLimited already called by fetchWithKeyRotation */
    },
    onUpstreamError: () => {
      /* forwarded by fetchWithKeyRotation */
    },
  });
}
...
function logChatDiagnostics(urlPath, body, bodyStream, reqId, startTime) {
  ...
  try {
    ...
    console.log(
      `[CHAT ${reqId}] mode=${streamMode} model=${parsed.model || parsed.messages?.[0]?.role || "?"}`,
    );
  } catch {
    /* non-JSON body */
  }
  ...
  bodyStream.on("data", (chunk) => {
    ...
    if (firstChunk) {
      firstChunk = false;
      console.log(
        `[CHAT ${reqId}] first chunk +${Date.now() - startTime}ms (${chunk.length}B)`,
      );
    }
    try {
      ...
      for (const line of lines) {
        ...
        if (obj.done === true) {
          console.log(
            `[CHAT ${reqId}] done=true received after ${chunkCount} chunks +${Date.now() - startTime}ms`,
          );
        }
      }
    } catch {
      /* chunk may be partial JSON — fine */
    }
  });

  bodyStream.on("end", () => {
    console.log(
      `[CHAT ${reqId}] stream ended — ${chunkCount} chunks total +${Date.now() - startTime}ms`,
    );
  });
}
